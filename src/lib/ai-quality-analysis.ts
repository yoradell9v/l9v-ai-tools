...
export async function analyzeKnowledgeBaseQuality(
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[AI Quality Analysis] Failed to parse AI response:",
        parseError
      );
...
function buildAnalysisPrompt(
  filledFields: Array<{ name: string; value: any }>,
  knowledgeBase: any
): string {
...
function processAIResponse(
  aiResponse: any,
  filledFields: Array<{ name: string; value: any }>
): Omit<AIQualityAnalysis, "analyzedAt"> {
  const fieldAnalysis: Record<string, FieldQualityAnalysis> = {};

  if (
    aiResponse.fieldAnalysis &&
    typeof aiResponse.fieldAnalysis === "object"
  ) {
    for (const [fieldName, analysis] of Object.entries(
      ...
      fieldAnalysis[fieldName] = {
        ...
        specificityScore: Math.min(
          100,
          Math.max(0, fieldData.specificityScore || 0)
        ),
        actionabilityScore: Math.min(
          100,
          Math.max(0, fieldData.actionabilityScore || 0)
        ),
        overallQuality: fieldData.overallQuality || "basic",
        strengths: Array.isArray(fieldData.strengths)
          ? fieldData.strengths
          : [],
        gaps: Array.isArray(fieldData.gaps) ? fieldData.gaps : [],
        recommendations: Array.isArray(fieldData.recommendations)
  ...
  const crossFieldCoherence = {
    score: Math.min(
      100,
      Math.max(0, aiResponse.crossFieldCoherence?.score || 0)
    ),
  ...
  const toolImpact = {
    jobDescriptionBuilder: processToolImpact(
      aiResponse.toolImpact?.jobDescriptionBuilder
    ),
  ...
  const topRecommendations: QualityRecommendation[] = Array.isArray(
...
export function getQualityDataForStorage(analysis: AIQualityAnalysis): any {
  return {
