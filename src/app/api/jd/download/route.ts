...
export async function POST(request: Request) {
  try {
    ...
    const serviceType =
      ...
      fullPackage?.service_structure?.service_type ||
      "";
    ...
    if (serviceType === "Dedicated VA") {
      ...
      if (dedicatedRole?.hours_per_week || detailedJd?.hours_per_week) {
        ...
        drawLine(
          String(
            dedicatedRole?.hours_per_week || detailedJd?.hours_per_week || ""
          ),
          { size: 11 }
        );
        y -= 5;
      ...
      if (detailedJd?.mission_statement || dedicatedRole?.core_responsibility) {
        ...
        drawLine(
          detailedJd?.mission_statement ||
            dedicatedRole?.core_responsibility ||
            "",
          { size: 11 }
        );
        y -= 5;
      ...
      if (
        detailedJd?.responsibilities &&
        detailedJd.responsibilities.length > 0
      ) {
        ...
        dedicatedRole?.task_allocation?.from_intake &&
        dedicatedRole.task_allocation.from_intake.length > 0
      ) {
      ...
      if (dedicatedRole?.skill_requirements || detailedJd?.skills_required) {
        const skills =
          dedicatedRole?.skill_requirements || detailedJd?.skills_required;
      ...
      if (
        dedicatedRole?.workflow_ownership &&
        dedicatedRole.workflow_ownership.length > 0
      ) {
      ...
      if (dedicatedRole?.interaction_model) {
        ...
        if (interaction.timezone_criticality) {
          drawLine(
            `Timezone criticality: ${interaction.timezone_criticality}`,
            { size: 11 }
          );
        }
        y -= 5;
      ...
      if (coreRole?.hours_per_week || detailedJd?.hours_per_week) {
        drawLine("Hours Per Week", { size: 12, bold: true });
        drawLine(
          String(coreRole?.hours_per_week || detailedJd?.hours_per_week || ""),
          { size: 11 }
        );
        y -= 5;
      }
      ...
      if (detailedJd?.mission_statement || coreRole?.core_responsibility) {
        drawLine("Mission Statement", { size: 12, bold: true });
        drawLine(
          detailedJd?.mission_statement || coreRole?.core_responsibility || "",
          { size: 11 }
        );
        y -= 5;
      }
      ...
      if (
        ...
        coreRole?.recurring_tasks &&
        coreRole.recurring_tasks.length > 0
      ) {
      ...
      if (coreRole?.skill_requirements || detailedJd?.skills_required) {
        const skills =
          coreRole?.skill_requirements || detailedJd?.skills_required;
        if (skills && typeof skills === "object") {
          if (
            skills.required &&
            Array.isArray(skills.required) &&
      ...
      if (
        coreRole?.workflow_ownership &&
        coreRole.workflow_ownership.length > 0
      ) {
        drawLine("Workflow Ownership", { size: 12, bold: true });
      ...
      if (coreRole?.interaction_model) {
        ...
        if (interaction.client_facing !== undefined) {
          drawLine(
            `Client-facing: ${interaction.client_facing ? "Yes" : "No"}`,
            { size: 11 }
          );
        }
        if (interaction.sync_needs) {
      ...
      const projects =
        fullPackage?.detailed_specifications?.projects ||
        fullPackage?.service_structure?.projects ||
        [];
      ...
      if (projects.length > 0) {
        for (let i = 0; i < projects.length; i++) {
          ...
          drawLine(
            `Project ${i + 1}: ${project.project_name || "Unnamed Project"}`,
            { size: 12, bold: true }
          );
          y -= 3;
          ...
          const skills =
            ...
            project.requirements?.skills_needed ||
            [];
          if (skills.length > 0) {
          ...
          if (
            ...
            Array.isArray(project.success_metrics) &&
    ...
    if (
      fullPackage?.service_structure?.team_support_areas &&
      Array.isArray(fullPackage.service_structure.team_support_areas) &&
      fullPackage.service_structure.team_support_areas.length > 0
    ) {
      ...
      for (const area of fullPackage.service_structure.team_support_areas) {
        if (area.skill_category) {
          ...
          if (area.estimated_hours_monthly) {
            drawLine(`Estimated hours/month: ${area.estimated_hours_monthly}`, {
              size: 11,
            });
          }
          if (
            area.use_cases &&
            Array.isArray(area.use_cases) &&
          ...
          if (
            area.deliverables &&
            Array.isArray(area.deliverables) &&
    ...
    if (
      fullPackage?.service_structure?.pros &&
      fullPackage.service_structure.pros.length > 0
    ) {
      drawSection("PROS");
    ...
    if (
      fullPackage?.service_structure?.cons &&
      fullPackage.service_structure.cons.length > 0
    ) {
      drawSection("CONS");
    ...
    if (fullPackage?.implementation_plan) {
      ...
      if (
        implPlan.immediate_next_steps &&
        implPlan.immediate_next_steps.length > 0
      ) {
        ...
        for (const step of implPlan.immediate_next_steps) {
          ...
          if (typeof step === "object" && step.owner) {
            drawLine(`Owner: ${step.owner} | Timeline: ${step.timeline}`, {
              size: 10,
            });
            if (step.output) {
      ...
      if (implPlan.onboarding_roadmap) {
        ...
        const hasContent =
          (roadmap.week_1 &&
            Array.isArray(roadmap.week_1) &&
            roadmap.week_1.length > 0) ||
          (roadmap.week_2 &&
            Array.isArray(roadmap.week_2) &&
            roadmap.week_2.length > 0) ||
          (roadmap.week_3_4 &&
            Array.isArray(roadmap.week_3_4) &&
            roadmap.week_3_4.length > 0) ||
          (roadmap.project_kickoff &&
            typeof roadmap.project_kickoff === "object") ||
          (roadmap.execution_phase &&
            typeof roadmap.execution_phase === "object") ||
          ...
          if (serviceType === "Projects on Demand") {
            ...
            for (const [key, value] of projectMilestones) {
              ...
              if (match) {
                ...
                const milestoneType =
                  match[2] === "kickoff" ? "Kickoff" : "Completion";
                drawLine(`Project ${projectNum} ${milestoneType}:`, {
                  size: 11,
                  bold: true,
                });
